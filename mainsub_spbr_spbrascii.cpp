...
int mainsub_spbr_spbrascii ( int argc, char** argv )
{
    ...
    if ( spbr_engine->isLuminanceAdjustment() ) {
        return mainsub_luminance_adjustment(   
                ...
                /* SPBR*                    */  spbr_engine, 
                /* kvs::PointObject*        */  object,
                /* FILE_FORMAT4LA           */  LuminanceAdjustment::SPBR_ASCII4LA );
    }

#if KVS_VERSION_MAJOR == 1
    kvs::glew::rits::ParticleBasedRenderer* renderer
        = new kvs::glew::rits::ParticleBasedRenderer();//KVS1
#elif KVS_VERSION_MAJOR == 2
    kvs::glsl::ParticleBasedRenderer* renderer
        = new kvs::glsl::ParticleBasedRenderer();//KVS2
#endif
